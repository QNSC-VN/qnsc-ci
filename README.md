...
jobs:
  quality:
    ...
    steps:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/setup-node-pnpm@main
        with:
          node-version: '22'
  ...
  openapi:                          # rally-api only
    ...
    steps:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/publish-openapi-spec@main
        with:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/validate-openapi-contract@main
        with:
...
jobs:
  deploy:
    ...
    steps:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/setup-aws-oidc@main
        id: aws
        with:
      ...
      # 2. Build & push image
      - uses: QNSC-VN/qncs-gitops/actions/build-push-ecr@main
        id: build
        with:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/attest-image@main
        with:
          image-ref: ${{ steps.build.outputs.image-uri }}

      # 4. Migrate DB (MUST run before new app version goes live)
      - uses: QNSC-VN/qncs-gitops/actions/run-db-migration@main
        with:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/verify-ecs-deploy@main
        with:
          cluster: ${{ vars.ECS_CLUSTER }}
          service: ${{ vars.ECS_API_SERVICE }}
          image-tag: ${{ env.IMAGE_TAG }}
      ...
      - uses: QNSC-VN/qncs-gitops/actions/post-deploy-health-check@main
        with:
          url: https://api.rally.io/v1/health/ready
          expected-version: ${{ env.IMAGE_TAG }}

      # 8. Notify result
      - uses: QNSC-VN/qncs-gitops/actions/notify-deploy@main
        if: always()
        with:
...
jobs:
  deploy:
    ...
    steps:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/cloudfront-invalidate@main
        with:
...
jobs:
  secrets:
    ...
    steps:
      ...
      - uses: QNSC-VN/qncs-gitops/actions/scan-secrets@main
    ...
    steps:
      - uses: QNSC-VN/qncs-gitops/actions/setup-aws-oidc@main
        with:
          ...
          region: ${{ vars.AWS_REGION }}
      - uses: QNSC-VN/qncs-gitops/actions/ecr-cleanup@main
        with:
          repositories: |
            rally-api
            rally-worker
...
uses: QNSC-VN/qncs-gitops/actions/setup-node-pnpm@v1
